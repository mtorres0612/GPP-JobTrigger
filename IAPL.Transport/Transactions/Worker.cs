 ...
 *  Change Log:
 * 
 *  MNTORRES 09162016 : Added serialization method
 * 
 */
using System;
...
namespace IAPL.Transport.Transactions
{
    class Worker
    {
        ...
        private void Serialize(List<string> files, string threadName, ServerDetails serverDetails, MessageDetails msgDetails, string type, int zipCopyToDestination)
        {
            string xmlFile                  = @"\\WLAP0135\Shared\" + msgDetails.MessageCode + "_" + type + ".xml";
            msgDetails.ZipCopyToDestination = zipCopyToDestination;
            serverDetails.MessageDetails    = msgDetails;
            serverDetails.Files             = files;
            serverDetails.ThreadName        = threadName;

            XmlSerializer x = new XmlSerializer(serverDetails.GetType());
            ...
            {
                x.Serialize(file, serverDetails, namespaces);
            }
        }
        #endregion
        ...
        internal void Serialize(DataTable dTable)
        {
            string xmlFile         = @"\\WLAP0135\Shared\JobSchedulerXMLs\MESSAGECODES_" + DateTime.Now.ToString("MM-dd-yyyy") + ".xml";
            List<Trigger> lstABCD  = new List<Trigger>();
            List<Trigger> lstEFGH  = new List<Trigger>();
            List<Trigger> lstIJKL  = new List<Trigger>();
            List<Trigger> lstMNOP  = new List<Trigger>();
            List<Trigger> lstQRST  = new List<Trigger>();
            List<Trigger> lstUVWX  = new List<Trigger>();
            List<Trigger> lstYZ09  = new List<Trigger>();
            GPP gpp                = new GPP();
            Jobs job               = new Jobs();

            foreach (DataRow row in dTable.Rows)
            {
                Trigger message         = new Trigger();
                message.MessageCode     = IAPL.Transport.Util.CommonTools.ValueToString((Object)row["MsgCode"]);
                message.ERP             = IAPL.Transport.Util.CommonTools.ValueToString((Object)row["ERP"]);
                message.PrincipalCode   = IAPL.Transport.Util.CommonTools.ValueToString((Object)row["PRNCPL"]);
                message.TriggerName     = message.MessageCode + "-" + message.ERP + "-" + message.PrincipalCode + "-" + Guid.NewGuid();
                message.MsgMonday       = Convert.ToInt32((Object)row["MsgMonday"]);
                message.MsgTuesday      = Convert.ToInt32((Object)row["MsgTuesday"]);
                message.MsgWednesday    = Convert.ToInt32((Object)row["MsgWednesday"]);
                message.MsgThursday     = Convert.ToInt32((Object)row["MsgThursday"]);
                message.MsgFriday       = Convert.ToInt32((Object)row["MsgFriday"]);
                message.MsgSaturday     = Convert.ToInt32((Object)row["MsgSaturday"]);
                message.MsgSunday       = Convert.ToInt32((Object)row["MsgSunday"]);
                message.MsetRunTime     = Convert.ToInt32((Object)row["MsetRunTime"]);
                message.MsetStartTime   = (DateTime)DateTime.Parse(IAPL.Transport.Util.CommonTools.ValueToString((Object)row["MsetStartTime"]));
                message.MsetEndTime     = (DateTime)DateTime.Parse(IAPL.Transport.Util.CommonTools.ValueToString((Object)row["MsetEndTime"]));
                message.JobCronSchedule = GetJobCronSchedule(row);
                message.RepeatForever   = 1;
                message.StartNow        = 0;


                if (message.MessageCode.StartsWith("A", StringComparison.CurrentCultureIgnoreCase) ||
                    message.MessageCode.StartsWith("B", StringComparison.CurrentCultureIgnoreCase) ||
                    message.MessageCode.StartsWith("C", StringComparison.CurrentCultureIgnoreCase) ||
                    message.MessageCode.StartsWith("D", StringComparison.CurrentCultureIgnoreCase))
                {
                    lstABCD.Add(message);
                }
                else if (message.MessageCode.StartsWith("E", StringComparison.CurrentCultureIgnoreCase) ||
                         message.MessageCode.StartsWith("F", StringComparison.CurrentCultureIgnoreCase) ||
                         message.MessageCode.StartsWith("G", StringComparison.CurrentCultureIgnoreCase) ||
                         message.MessageCode.StartsWith("H", StringComparison.CurrentCultureIgnoreCase))
                {
                    lstEFGH.Add(message);
                }
                else if (message.MessageCode.StartsWith("I", StringComparison.CurrentCultureIgnoreCase) ||
                         message.MessageCode.StartsWith("J", StringComparison.CurrentCultureIgnoreCase) ||
                         message.MessageCode.StartsWith("K", StringComparison.CurrentCultureIgnoreCase) ||
                         message.MessageCode.StartsWith("L", StringComparison.CurrentCultureIgnoreCase))
                {
                    lstIJKL.Add(message);
                }
                else if (message.MessageCode.StartsWith("M", StringComparison.CurrentCultureIgnoreCase) ||
                         message.MessageCode.StartsWith("N", StringComparison.CurrentCultureIgnoreCase) ||
                         message.MessageCode.StartsWith("O", StringComparison.CurrentCultureIgnoreCase) ||
                         message.MessageCode.StartsWith("P", StringComparison.CurrentCultureIgnoreCase))
                {
                    lstMNOP.Add(message);
                }
                else if (message.MessageCode.StartsWith("Q", StringComparison.CurrentCultureIgnoreCase) ||
                         message.MessageCode.StartsWith("R", StringComparison.CurrentCultureIgnoreCase) ||
                         message.MessageCode.StartsWith("S", StringComparison.CurrentCultureIgnoreCase) ||
                         message.MessageCode.StartsWith("T", StringComparison.CurrentCultureIgnoreCase))
                {
                    lstQRST.Add(message);
                }
                else if (message.MessageCode.StartsWith("U", StringComparison.CurrentCultureIgnoreCase) ||
                         message.MessageCode.StartsWith("V", StringComparison.CurrentCultureIgnoreCase) ||
                         message.MessageCode.StartsWith("W", StringComparison.CurrentCultureIgnoreCase) ||
                         message.MessageCode.StartsWith("X", StringComparison.CurrentCultureIgnoreCase))
                {
                    lstUVWX.Add(message);
                }
                else
            ...
            ABCD abcd                          = new ABCD();
            abcd.Triggers                      = lstABCD;
            EFGH efgh                          = new EFGH();
            efgh.Triggers                      = lstEFGH;
            IJKL ijkl                          = new IJKL();
            ijkl.Triggers                      = lstIJKL;
            MNOP mnop                          = new MNOP();
            mnop.Triggers                      = lstMNOP;
            QRST qrst                          = new QRST();
            qrst.Triggers                      = lstQRST;
            UVWX uvwx                          = new UVWX();
            uvwx.Triggers                      = lstUVWX;
            YZ09 yz09                          = new YZ09();
            yz09.Triggers                      = lstYZ09;
            job.ABCD                           = abcd;
            job.EFGH                           = efgh;
            job.IJKL                           = ijkl;
            job.MNOP                           = mnop;
            ...
            job.YZ09                           = yz09;
            gpp.Job                            = job;
            XmlSerializer x                    = new XmlSerializer(gpp.GetType());
            XmlSerializerNamespaces namespaces = new XmlSerializerNamespaces();
            ...
            using (FileStream file = File.Create(xmlFile))
            {
                x.Serialize(file, gpp, namespaces);
            }
        ...
        private string GetJobCronSchedule(DataRow row)
        {
            ...
            if (Convert.ToInt32((Object)row["MsgTuesday"]) == 1)
            {
                sb.Append("TUE,");
            }
            if (Convert.ToInt32((Object)row["MsgWednesday"]) == 1)
            {
                sb.Append("WED,");
            }
            if (Convert.ToInt32((Object)row["MsgThursday"]) == 1)
            {
                sb.Append("THU,");
            }
            if (Convert.ToInt32((Object)row["MsgFriday"]) == 1)
            {
                sb.Append("FRI,");
            }
            if (Convert.ToInt32((Object)row["MsgSaturday"]) == 1)
            {
                sb.Append("SAT,");
            }
            if (Convert.ToInt32((Object)row["MsgSunday"]) == 1)
            {
                sb.Append("SUN,");
            }

            DateTime startTime = (DateTime)DateTime.Parse(IAPL.Transport.Util.CommonTools.ValueToString((Object)row["MsetStartTime"]));
            ...
            arrValues[4] = "*"; //month
            arrValues[5] = sb.ToString().TrimEnd(',');  //day of week
            arrValues[6] = "*"; //year
